...
import { initCsvFile, appendProductToCsv, csvFilename } from "./csv-writer.js";
...
// Establish PostgreSQL connection if being used
if (databaseMode) establishPostgresDB();

// Create a fresh CSV file if save mode is on
if (saveMode) initCsvFile();
...
async function processFoundProductEntries(
  ...
  for (let i = 0; i < productEntries.length; i++) {
    ...
    if (databaseMode && product !== undefined) {
      ...
      if (saveMode) appendProductToCsv(product);
    }
...
function handleArguments(categorisedUrls: CategorisedUrl[]): CategorisedUrl[] {
  if (process.argv.length > 2) {
    ...
    userArgs.forEach(async (arg) => {
      ...
      else if (arg === "save") saveMode = true;
